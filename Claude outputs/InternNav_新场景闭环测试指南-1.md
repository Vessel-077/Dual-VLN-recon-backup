...
```bash
cd ~/InternNav   # your InternNav repo root
...
```python
#!/usr/bin/env python
"""Generate a minimal R2R-format episode file for closed-loop smoke testing on a new scene (default: skokloster-castle)."""
import argparse
import gzip
import json
import math
...
def sample_episode_pair(pathfinder, min_dist=3.0, max_dist=12.0, max_tries=200):
    for _ in range(max_tries):
        ...
        if min_dist <= path.geodesic_distance <= max_dist:
            return p0, p1, path
    raise RuntimeError("Failed to find a valid start/goal pair within the distance bounds after many tries; try widening min_dist/max_dist")


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--scene-id", default="skokloster-castle",
                     help="the folder name under data/scene_data/mp3d_ce/mp3d/<scene-id>/")
    ap.add_argument("--navmesh", default=None,
                     help="defaults to data/scene_data/mp3d_ce/mp3d/<scene-id>/<scene-id>.navmesh")
    ap.add_argument("--out", default=None,
                     help="defaults to data/vln_ce/raw_data/r2r_custom_scene/val_mini/val_mini.json.gz")
    ...
    assert pf.is_loaded, f"failed to load navmesh: {navmesh}"
...
```python
    ...
    eval_settings={
        ...
        "save_video": True,          # recommended: keep it on to visually confirm the agent actually moves in the new scene on the first run
        "epoch": 0,
        "max_steps_per_episode": 200, # a smoke test doesn't need as many as 500
        "port": "2333",
        "dist_url": "env://",
    },
```
...
with habitat.Env(config=config) as env:
    print("number of episodes loaded:", len(env.episodes))
    obs = env.reset()
    print("observation keys:", list(obs.keys()), "rgb shape:", obs["rgb"].shape)
    for _ in range(10):
        obs = env.step("move_forward")
    print("metrics after 10 steps:", env.get_metrics())
```
...
流程完全一样,唯一的区别是第 1 步换成:**只导入 `.glb`,自己现场烘焙 navmesh —— 这是标准做法,不是变通方案。**官方 habitat_test_scenes / MP3D 之所以自带 `.navmesh`,只是因为别人已经帮你烘焙好了;自建场景没有这份"别人烘焙好的"文件,本来就该自己算。

1. 用 Blender / 扫描重建工具导出 `.glb`(注意坐标系:habitat 默认 **Y-up**,如果你的建模工具是 Z-up 需要在导出时转换,否则导航网格会整体歪掉、agent 会一直"悬空"或"陷地"里出不来)。
2. 用 habitat-sim 重新烘焙导航网格。**有一处容易踩的坑**:我查了 `habitat-lab` 里的默认 agent 配置(`AgentConfig` dataclass),你现在 `vln_r2r.yaml` 里实际生效的 agent 是 `height=1.5m, radius=0.1m`(yaml 里没有覆盖这两个字段)。烘焙 navmesh 时如果不显式对齐这两个值,`NavMeshSettings()` 的默认值虽然凑巧也是 `radius=0.1 / height=1.5 / max_climb=0.2 / max_slope=45°`,但**只有调用了 `set_defaults()` 才会真正生效**,直接 `NavMeshSettings()` 构造出来的对象各字段是 0,烘焙出来的 navmesh 会退化成不可用的空壳:

   ```python
   import habitat_sim

   scene_glb = "path/to/your_scene.glb"
   ...
   navmesh_settings.set_defaults()      # critical: without this call, the fields below default to 0
   navmesh_settings.agent_radius = 0.1  # match the agent actually in effect in vln_r2r.yaml
   navmesh_settings.agent_height = 1.5

   ok = sim.recompute_navmesh(sim.pathfinder, navmesh_settings)
   assert ok, "navmesh baking failed; check the glb's coordinate system/units first"

   navmesh_path = scene_glb.replace(".glb", ".navmesh")
   sim.pathfinder.save_nav_mesh(navmesh_path)
   sim.close()
   print("navmesh saved to:", navmesh_path)
   ```

3. **烘焙完先肉眼/程序核查一遍,再往下走**,自建场景比官方场景更容易出现"看起来加载成功但导航网格是碎的"这种情况(常见原因:多个不连通的孤岛、法线朝向反了导致地面没被识别成可走面、单位是厘米而不是米导致整个场景被判定成一个几十米高的房间):
   ```python
   print("navmesh loaded successfully:", sim.pathfinder.is_loaded)
   print("navigable area (m^2):", sim.pathfinder.navigable_area)
   pts = [sim.pathfinder.get_random_navigable_point() for _ in range(20)]
   print("20 randomly sampled navigable points:", pts[:3], "...")
   ```
   `navigable_area` 是 0 或采样点全部报错,基本可以断定是坐标系/单位问题,回第 1 步检查导出设置。
4. 从这里开始,回到本指南第 3 步,把 `.glb` + 刚烘焙出来的 `.navmesh` 放进 `data/scene_data/mp3d_ce/mp3d/<你的场景名>/`,后面第 4-8 步一字不改就能复用 —— 第 4 步的 `make_custom_scene_episodes.py` 本身就是靠 `pathfinder.get_random_navigable_point()` + `find_path` 采样,如果 navmesh 是坏的,那一步会直接报错或者反复采样失败,正好也是一次额外的自建场景校验。
