...
class HabitatEnv(base.Env):
    ...
    def generate_episodes(self) -> List[Any]:
        ...
        # ---- ADDED: optional allow-list filter (e.g. only "turn around" episodes) ----
        allowed_pairs = None
        filter_path = os.environ.get("INTERNNAV_EPISODE_FILTER")
        if filter_path and os.path.exists(filter_path):
            with open(filter_path, 'r') as f:
                allowed_pairs = {(str(s), int(e)) for s, e in json.load(f)}
        ...
        for scene in sorted(scene_episode_dict.keys()):
            ...
            for episode in per_scene_eps[self.rank :: self.world_size]:
                ...
                if (scene_id, episode_id) in done_res:
                    continue
                if allowed_pairs is not None and (scene_id, episode_id) not in allowed_pairs:  #ADDED: optional allow-list filter
                    continue
