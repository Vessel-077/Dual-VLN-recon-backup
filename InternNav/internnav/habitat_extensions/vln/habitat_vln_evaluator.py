...
class HabitatVLNEvaluator(DistributedEvaluator):
    def __init__(self, cfg: EvalCfg):
        ...
        if self.model_args.mode == 'dual_system':
            from transformers import BitsAndBytesConfig     #added due to limited GPU memory
            model = InternVLAN1ForCausalLM.from_pretrained(
                self.model_args.model_path,
                torch_dtype=torch.bfloat16,
                attn_implementation="flash_attention_2",
                device_map={"": device},
                quantization_config=BitsAndBytesConfig(         #added due to limited GPU memory
                    #load_in_8bit=True,                         #we actually need 4-bit because simulation in Habitat takes up a bit of memory as well
                    load_in_4bit=True,
                    bnb_4bit_compute_dtype=torch.bfloat16,
                    bnb_4bit_quant_type="nf4",
                    llm_int8_skip_modules=[
                        "rgb_model", "memory_encoder", "rgb_resampler",
                        "traj_dit", "action_encoder", "action_decoder",
                        "cond_projector", "navdp", "lm_head",
                    ],
                ),
            )
    ...
    def _run_eval_dual_system(self) -> tuple:  # noqa: C901
        ...
        while self.env.is_running:
            ...
            # ---- ADDED: optional pure-camera (no overlay at all) video ----
            pure_cam_writer = None
            pure_cam_dir = os.environ.get("INTERNNAV_PURE_CAM_VIDEO_DIR")
            if pure_cam_dir:
                os.makedirs(pure_cam_dir, exist_ok=True)
                pure_cam_writer = imageio.get_writer(
                    os.path.join(pure_cam_dir, f'{scene_id}_{episode_id:04d}.mp4'),
                    fps=int(os.environ.get("INTERNNAV_PURE_CAM_FPS", "6")),
                )
            ...
            if vis_writer is not None:
                vis_writer.close()
            if pure_cam_writer is not None:     #close pure camera writer
                pure_cam_writer.close()
